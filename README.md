...
and `SECURITY.md` and `DATA-PRIVACY.md` for the full posture. The design is
also written up as a named pattern — the **Zero-Custody architecture** — in
`docs/whitepaper-zero-custody.md`.
...
Third-party dependency licenses (all permissive) are inventoried in
`THIRD-PARTY-LICENSES.md`, regenerated by `npm run licenses:doc` and gated in
CI by `npm run licenses:check`.
